...
public class mainpage extends JPanel {
	...
	// ---- Add Song fields ----
	private JTextField addArtistKeyField, addCountryField,
	...
	private JComboBox<String> updateSetColumnCombo;
	private JTextField updateWhereValueField;
	private JTextField updateSetValueField;

	private static final String[] ARTIST_COLUMNS = { "ARTIST", "COUNTRY" };
	...
	private JPanel buildAddSongPanel() {
		...
		lblCountry.setBounds(50, 100, 120, 20);
		panel.add(lblCountry);
		addCountryField = new JTextField();
		addCountryField.setBounds(190, 100, 150, 25);
		panel.add(addCountryField);

		JLabel lblAlbumKey = new JLabel("Album Name:");
		lblAlbumKey.setBounds(50, 150, 120, 20);
		panel.add(lblAlbumKey);
		addAlbumKeyField = new JTextField();
		addAlbumKeyField.setBounds(190, 150, 150, 25);
		...
		JLabel lblSongKey = new JLabel("Song Name:");
	...
	private void handleAddSong() {
		...
		try {
			...
			upsertArtist(conn, artistKey, country);
	...
	private void upsertArtist(Connection conn, String artistKey, String country) throws SQLException {
		String sql = """
			MERGE INTO artists a
			USING (SELECT ? AS artist FROM dual) src
			ON (a.artist = src.artist)
			WHEN MATCHED THEN UPDATE SET a.country = ?
			WHEN NOT MATCHED THEN INSERT (artist, country) VALUES (src.artist, ?)
		""";
	...
	private JPanel buildUpdatePanel() {
		...
		JLabel lblSetColumn = new JLabel("Column to set (SET):");
		lblSetColumn.setBounds(50, 200, 160, 20);
		panel.add(lblSetColumn);

		updateSetColumnCombo = new JComboBox<>(ARTIST_COLUMNS);
		updateSetColumnCombo.setBounds(220, 200, 180, 25);
		panel.add(updateSetColumnCombo);

		JLabel lblSetValue = new JLabel("New value to set (SET):");
		lblSetValue.setBounds(50, 250, 160, 20);
		panel.add(lblSetValue);

		updateSetValueField = new JTextField();
		updateSetValueField.setBounds(220, 250, 180, 25);
		panel.add(updateSetValueField);

		JButton submit = new JButton("Update");
		submit.setBounds(220, 310, 100, 30);
		submit.addActionListener(e -> handleUpdate());
		panel.add(submit);

		JButton back = new JButton("Back");
		back.setBounds(340, 310, 80, 30);
		back.addActionListener(e -> cardLayout.show(cardPanel, "MENU"));
		panel.add(back);
	...
	// Swaps both column dropdowns' options to match whichever table is selected.
	private void refreshUpdateColumns() {
		...
		updateSetColumnCombo.setModel(new DefaultComboBoxModel<>(columns));
		updateWhereValueField.setText("");
		updateSetValueField.setText("");
	}

	// Builds "UPDATE <table> SET <setColumn> = ? WHERE <whereColumn> = ?" from
	// the table/columns picked in the combo boxes and the two typed-in values.
	private void handleUpdate() {
		...
		String whereColumn = (String) updateColumnCombo.getSelectedItem();
		String setColumn = (String) updateSetColumnCombo.getSelectedItem();
	...
	private JPanel buildViewTablesPanel() {
		...
		// until runtime.
		artistsModel = new DefaultTableModel(new Object[] { "ARTIST", "COUNTRY" }, 0);
	...
	private void loadArtists(Connection conn) {
		...
		String sql = "SELECT artist, country FROM artists";
		try (Statement stmt = conn.createStatement(); ResultSet rs = stmt.executeQuery(sql)) {
			while (rs.next()) {
				artistsModel.addRow(new Object[] {
						rs.getString("artist"), rs.getString("country") });
			}
		} catch (SQLException ex) {
